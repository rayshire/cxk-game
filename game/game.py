...
enemy3_down_sound = pygame.mixer.Sound("sound/explode.wav")
...
def ui():
    ...
    # 初始化游戏规则图片并获取图片的矩形位置
    rules_image = pygame.image.load("images/gamecontrol.png").convert_alpha()
...
def main():
    ...
    again_image = pygame.image.load("images/replay.png").convert_alpha()
    ...
    while True:
        ...
        if life_num and not paused:
            ...
            for each in big_enemies:
                if each.active:
                    ...
                    # 绘制血槽
                    pygame.draw.line(screen, YELLOW, (each.rect.left, each.rect.top - 5),
            ...
            # 剩余炸弹数量
            bomb_text = bomb_font.render("X%d" % bomb_num, True, YELLOW)
            ...
            # 绘制得分
            score_text = score_font.render("Score: %s" % str(score), True, YELLOW)
        ...
        # 绘制游戏结束画面
            ...
            # 绘制结束界面
            record_score_text = score_font.render("Best:%d" % record_score, True, YELLOW)
            screen.blit(record_score_text, (50, 40))

            gameover_text1 = gameover_font.render("Your Score", True, YELLOW)
            gameover_text1_rect = gameover_text1.get_rect()
            gameover_text1_rect.left, gameover_text1_rect.top = (width - gameover_text1_rect.width) // 2, height - 500
            screen.blit(gameover_text1, gameover_text1_rect)

            gameover_text2 = gameover_font.render(str(score), True, YELLOW)
